...
async fn sync_chain_monitor<'a, CL>(new_header: BlockHeaderData, old_header: &BlockHeaderData, block_source: &mut dyn BlockSource, chain_notifier: &mut BlockNotifier<'a, CL>, head_blocks: &mut Vec<BlockHeaderData>, mainnet: bool) -> Result<(), (BlockSourceRespErr, Option<BlockHeaderData>)>
	where CL: Deref<Target = dyn ChainListener + 'a> + 'a
{
...
pub async fn init_sync_chain_monitor<'a, CL, B>(new_block: BlockHash, old_block: BlockHash, block_source: &mut B, mut chain_notifier: BlockNotifier<'a, CL>)
	where CL: Deref<Target = dyn ChainListener + 'a> + 'a,
		  B: BlockSource,
{
...
pub struct MicroSPVClient<'a, 'b, B, CL>
	where B: DerefMut<Target=dyn BlockSource + 'a> + Sized + Sync + Send,
		  CL: Deref<Target = dyn ChainListener + 'b> + 'b,
{
	...
	chain_notifier: BlockNotifier<'b, CL>,
	mainnet: bool
}
impl<'a, 'b, B, CL> MicroSPVClient<'a, 'b, B, CL>
	where B: DerefMut<Target=dyn BlockSource + 'a> + Sized + Sync + Send,
		  CL: Deref<Target = dyn ChainListener + 'b> + 'b,
{
	...
	pub fn init(chain_tip: BlockHeaderData, block_sources: Vec<B>, backup_block_sources: Vec<B>, chain_notifier: BlockNotifier<'b, CL>, mainnet: bool) -> Self {
...
mod tests {
	...
	impl ChainListener for TestChainListener {
		fn block_connected(&self, header: &BlockHeader, txdata: &[(usize, &Transaction)], height: u32) {
			self.blocks_connected.lock().unwrap().push((header.bitcoin_hash(), height));
		}
	...
	async fn simple_block_connect() {
		...
		assert_eq!(&listener.blocks_disconnected.lock().unwrap()[..], &[(block_1a_hash, 1)][..]);
		assert_eq!(&listener.blocks_connected.lock().unwrap()[..], &[(block_1b_hash, 1), (block_2b_hash, 2)][..]);
		...
		listener.blocks_connected.lock().unwrap().clear();
		listener.blocks_disconnected.lock().unwrap().clear();

		// First test that nothing happens if nothing changes:
		assert!(!client.poll_best_tip().await);
		assert!(listener.blocks_disconnected.lock().unwrap().is_empty());
		assert!(listener.blocks_connected.lock().unwrap().is_empty());
		...
		*chain_two.best_block.lock().unwrap() = (block_3a_hash, Some(3));
		...
		assert!(client.poll_best_tip().await);
		assert_eq!(&listener.blocks_disconnected.lock().unwrap()[..], &[(block_2b_hash, 2), (block_1b_hash, 1)][..]);
		assert_eq!(&listener.blocks_connected.lock().unwrap()[..], &[(block_1a_hash, 1), (block_2a_hash, 2), (block_3a_hash, 3)][..]);
		...
		*backup_chain.best_block.lock().unwrap() = (block_4a_hash, Some(4));
		...
		assert!(client.poll_best_tip().await);
		assert!(listener.blocks_disconnected.lock().unwrap().is_empty());
		assert_eq!(&listener.blocks_connected.lock().unwrap()[..], &[(block_4a_hash, 4)][..]);
		...
		*backup_chain.best_block.lock().unwrap() = (block_3a_hash, Some(3));
		...
		assert!(client.poll_best_tip().await);
		assert_eq!(&listener.blocks_disconnected.lock().unwrap()[..], &[(block_4a_hash, 4)][..]);
		assert!(listener.blocks_connected.lock().unwrap().is_empty());
