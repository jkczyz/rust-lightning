...
fn packet_payloads_and_keys<T: OnionMessageContents, S: secp256k1::Signing + secp256k1::Verification>(
	...
	utils::construct_keys_callback(secp_ctx, unblinded_path.into_iter().map(|pubkey| (pubkey, None)), Some(destination), session_priv,
		|_, onion_packet_ss, ephemeral_pubkey, control_tlvs_ss, unblinded_pk_opt, enc_payload_opt, _: Option<()>| {
