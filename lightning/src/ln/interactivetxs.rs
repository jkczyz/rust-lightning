...
impl HandleTxCompleteValue {
	pub fn into_msg_send_event(
		...
		match self {
			...
			HandleTxCompleteValue::SendTxComplete(msg, ready_to_sign) => {
				(Some(msg.into_msg_send_event(counterparty_node_id)), ready_to_sign)
			},
				...
				match msg {
					...
					InteractiveTxMessageSend::TxComplete(msg) => {
						for_constructor.handle_tx_complete(&msg).map(|value| match value {
							...
							HandleTxCompleteValue::SendTxComplete(msg_send, ready_to_sign) => {
								(Some(msg_send), ready_to_sign)
							},
							HandleTxCompleteValue::NegotiationComplete => (None, true),
						})
					},
