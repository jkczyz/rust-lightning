	...
	<SP::Target as SignerProvider>::EcdsaSigner: EcdsaChannelSigner,
{
	...
	where L::Target: Logger
	{
		...
		let current_holder_commitment_point = &mut self.current_holder_commitment_point.clone();
		...
		self.current_holder_commitment_point = *current_holder_commitment_point;
		...
		L::Target: Logger,
	{
		...
		let update = self
			.context
			.validate_commitment_signed(&self.funding, &self.current_holder_commitment_point, msg, logger)
	...
	SP::Target: SignerProvider,
{
	fn write<W: Writer>(&self, writer: &mut W) -> Result<(), io::Error> {
		...
		let cur_holder_commitment_point = Some(self.current_holder_commitment_point.current_point());
