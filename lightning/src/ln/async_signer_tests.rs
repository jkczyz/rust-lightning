...
fn do_test_closing_signed(extra_closing_signed: bool, reconnect: bool) {
	...
	if extra_closing_signed {
		let node_1_closing_signed_2_bad = {
			...
			let channel = chan_lock.channel_by_id.get(&chan_id).unwrap();
			let funded = channel.as_funded().unwrap();
			let signer = funded.context.get_signer().as_ecdsa().unwrap();
			let signature = signer
				.sign_closing_transaction(
					&funded.get_channel_transaction_parameters(),
					&closing_tx_2,
					&Secp256k1::new(),
				)
				.unwrap();
