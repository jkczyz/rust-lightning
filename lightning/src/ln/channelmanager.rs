...
macro_rules! convert_channel_err {
	...
	($self: ident, $peer_state: expr, $err: expr, $funded_channel: expr, $channel_id: expr, FUNDED_CHANNEL) => {
		convert_channel_err!($self, $peer_state, $err, $funded_channel.context_mut(), $channel_id, MANUAL_CHANNEL_UPDATE, { $self.get_channel_update_for_broadcast(&$funded_channel).ok() })
	};
		...
		let (update_opt, counterparty_node_id) = {
			...
			if let hash_map::Entry::Occupied(mut chan_entry) = peer_state.channel_by_id.entry(channel_id.clone()) {
				...
				let (mut shutdown_res, update_opt) = match chan_entry.get_mut().as_funded_mut() {
					Some(chan) => {
						(
							chan.context_mut().force_shutdown(broadcast, closure_reason),
			...
			Some(Ok(mut chan)) => {
				macro_rules! close_chan { ($err: expr, $api_err: expr, $chan: expr) => { {
					...
					let err = if let ChannelError::Close((msg, reason)) = $err {
						...
						let shutdown_res = $chan.context_mut().force_shutdown(false, reason);
			...
			hash_map::Entry::Vacant(e) => {
				...
				match outpoint_to_peer.entry(funding_txo) {
					...
					hash_map::Entry::Occupied(o) => {
						...
						self.finish_close_channel(chan.context_mut().force_shutdown(true, reason));
					...
					let outpoint = match &funding {
						FundingType::Checked(tx) => {
							for (idx, outp) in tx.output.iter().enumerate() {
								if outp.script_pubkey == expected_spk && outp.value.to_sat() == chan.context().channel_value_satoshis() {
...
This indicates a bug inside LDK. Please report this error at https://github.com/lightningdevkit/rust-lightning/issues/new";
				...
				hash_map::Entry::Occupied(mut chan) => {
					match chan.get_mut().as_unfunded_outbound_v1_mut() {
						Some(unfunded_chan) => {
							...
							(unfunded_chan.context().channel_value_satoshis(), unfunded_chan.context.get_funding_redeemscript().to_p2wsh(), unfunded_chan.context.get_user_id())
						},
						None => {
	...
	fn internal_funding_created(&self, counterparty_node_id: &PublicKey, msg: &msgs::FundingCreated) -> Result<(), MsgHandleErrInternal> {
				...
				Some(Ok(inbound_chan)) => {
					...
					match inbound_chan.funding_created(msg, best_block, &self.signer_provider, &&logger) {
						...
						Err((mut inbound_chan, err)) => {
							...
							return Err(convert_channel_err!(self, peer_state, err, inbound_chan.context_mut(), &msg.temporary_channel_id, UNFUNDED_CHANNEL).1);
						},
					}
				},
				Some(Err(mut chan)) => {
		...
		macro_rules! fail_chan { ($err: expr) => { {
			...
			return Err(convert_channel_err!(self, peer_state, err, chan.context_mut(), &funded_channel_id, UNFUNDED_CHANNEL).1);
		} } }
					...
					match chan.as_funded_mut() {
						...
						Some(funded_channel) => {
							...
							if let Ok((channel_ready_opt, mut timed_out_pending_htlcs, announcement_sigs)) = res {
								...
								let mut close_res = funded_channel.context_mut().force_shutdown(true, reason);
		...
		for _ in 0..channel_count {
			...
			if let Some(ref mut monitor) = args.channel_monitors.get_mut(&channel_id) {
				if channel.get_cur_holder_commitment_transaction_number() > monitor.get_cur_holder_commitment_number() ||
					...
					let mut shutdown_result = channel.context_mut().force_shutdown(true, ClosureReason::OutdatedChannelManager);
					...
					channel_closures.push_back((events::Event::ChannelClosed {
						...
						channel_capacity_sats: Some(channel.context().channel_value_satoshis()),
				...
				let _ = channel.context_mut().force_shutdown(false, ClosureReason::DisconnectedPeer);
				channel_closures.push_back((events::Event::ChannelClosed {
					channel_id: channel.context.channel_id(),
					user_channel_id: channel.context.get_user_id(),
