...
impl wire::CustomMessageReader for IgnoringCustomMessageHandler {
	type CustomMessage = DummyCustomType;
	fn read<R: io::Read>(&self, _message_type: u16, _buffer: &mut R) -> Result<Option<Self::CustomMessage>, msgs::DecodeError> {
		Ok(None)
	}
}

impl CustomMessageHandler for IgnoringCustomMessageHandler {
...
impl Deref for IgnoringCustomMessageHandler {
	type Target = IgnoringCustomMessageHandler;
	fn deref(&self) -> &Self { self }
}
...
pub type SimpleRefPeerManager<'a, 'b, 'c, 'd, 'e, 'f, 'g, SD, M, T, F, C, L> = PeerManager<SD, SimpleRefChannelManager<'a, 'b, 'c, 'd, 'e, M, T, F, L>, &'e NetGraphMsgHandler<&'g C, &'f L>, &'f L, IgnoringCustomMessageHandler>;
...
impl<Descriptor: SocketDescriptor, CM: Deref, L: Deref> PeerManager<Descriptor, CM, IgnoringMessageHandler, L, IgnoringCustomMessageHandler> where
		CM::Target: ChannelMessageHandler,
		L::Target: Logger {
	...
	pub fn new_channel_only(channel_message_handler: CM, our_node_secret: SecretKey, ephemeral_random_data: &[u8; 32], logger: L) -> Self {
		...
		}, our_node_secret, ephemeral_random_data, logger, IgnoringCustomMessageHandler{})
	}
}

impl<Descriptor: SocketDescriptor, RM: Deref, L: Deref> PeerManager<Descriptor, ErroringMessageHandler, RM, L, IgnoringCustomMessageHandler> where
		RM::Target: RoutingMessageHandler,
		L::Target: Logger {
...
#[cfg(test)]
mod tests {
	use ln::peer_handler::{PeerManager, MessageHandler, SocketDescriptor, IgnoringCustomMessageHandler};
	use ln::msgs;
	use util::events;
	use util::test_utils;
	...
	fn create_network<'a>(peer_count: usize, cfgs: &'a Vec<PeerManagerCfg>) -> Vec<PeerManager<FileDescriptor, &'a test_utils::TestChannelMessageHandler, &'a test_utils::TestRoutingMessageHandler, &'a test_utils::TestLogger, IgnoringCustomMessageHandler>> {
		...
		for i in 0..peer_count {
			...
			let peer = PeerManager::new(msg_handler, node_secret, &ephemeral_bytes, &cfgs[i].logger, IgnoringCustomMessageHandler {});
			peers.push(peer);
		}
	...
	fn establish_connection<'a>(peer_a: &PeerManager<FileDescriptor, &'a test_utils::TestChannelMessageHandler, &'a test_utils::TestRoutingMessageHandler, &'a test_utils::TestLogger, IgnoringCustomMessageHandler>, peer_b: &PeerManager<FileDescriptor, &'a test_utils::TestChannelMessageHandler, &'a test_utils::TestRoutingMessageHandler, &'a test_utils::TestLogger, IgnoringCustomMessageHandler>) -> (FileDescriptor, FileDescriptor) {
