...
mod tests {
	...
	use ::ln::peer_handler::IgnoringCustomMessageHandler;
	...
	fn read_empty_buffer() {
		...
		assert!(read(&mut reader, &IgnoringCustomMessageHandler{}).is_err());
	}
	...
	fn read_message_encoded_with_write() {
		...
		let mut reader = io::Cursor::new(buffer);
		let decoded_message = read(&mut reader, &IgnoringCustomMessageHandler{}).unwrap();
	...
	fn check_init_msg(buffer: Vec<u8>, expect_unknown: bool) {
		let mut reader = io::Cursor::new(buffer);
		let decoded_msg = read(&mut reader, &IgnoringCustomMessageHandler{}).unwrap();
		match decoded_msg {
	...
	impl core::ops::Deref for TestCustomMessageReader {
		type Target = TestCustomMessageReader;
