...
fn do_test_counterparty_no_reserve(send_from_initiator: bool) {
	...
	nodes[0].node.handle_accept_channel(node_b_id, &accept_channel_message);
	{
		...
		if let Some(chan) = channel.as_unfunded_outbound_v1_mut() {
			chan.set_holder_selected_channel_reserve_satoshis(0);
...
pub fn do_test_fee_spike_buffer(cfg: Option<UserConfig>, htlc_fails: bool) {
	...
	let res = {
		...
		let complete_params = channel.as_funded().unwrap().get_channel_transaction_parameters();
...
pub fn do_test_dust_limit_fee_accounting(can_afford: bool) {
...

	{
		...
		assert_eq!(
			chan.as_funded().unwrap().get_holder_selected_channel_reserve_satoshis(),
			channel_reserve_satoshis
		);
	}
	{
