...
pub(super) fn construct_onion_packet(payloads: Vec<msgs::OnionHopData>, onion_keys: Vec<OnionKeys>, prng_seed: [u8; 32], associated_data: &PaymentHash) -> msgs::OnionPacket {
	...
	construct_onion_packet_with_init_noise::<_, _, msgs::OnionPacket>(
...
/// Information used to create an onion packet, see `Packet` below for more info.
...
pub(crate) trait Packet {
	type D;
	fn create(pubkey: PublicKey, hop_data: Self::D, hmac: [u8; 32]) -> Self;
}

/// panics if route_size_insane(payloads) and we're sending a payment
fn construct_onion_packet_with_init_noise<HD: Writeable, D: PacketData, P: Packet<D = D>>(mut payloads: Vec<HD>, onion_keys: Vec<OnionKeys>, mut packet_data: D, associated_data: Option<&PaymentHash>) -> P {
...
mod tests {
	...
	fn onion_vectors() {
		...
		let packet = super::construct_onion_packet_with_init_noise::<_, _, msgs::OnionPacket>(payloads, onion_keys, [0; super::ONION_DATA_LEN], Some(&PaymentHash([0x42; 32])));
