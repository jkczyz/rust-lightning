...
/// panics if route_size_insane(paylods) and we're constructing a packet for a payment
pub(super) fn construct_onion_packet(payloads: Vec<msgs::OnionHopData>, onion_keys: Vec<OnionKeys>, prng_seed: [u8; 32], associated_data: &PaymentHash) -> msgs::OnionPacket {
	...
	construct_onion_packet_with_init_noise::<_, msgs::OnionPacket>(
		payloads, onion_keys, PacketData::Payment(packet_data), Some(associated_data))
}
...
pub(super) fn construct_onion_packet_bogus_hopdata<HD: Writeable>(payloads: Vec<HD>, onion_keys: Vec<OnionKeys>, prng_seed: [u8; 32], associated_data: &PaymentHash) -> msgs::OnionPacket {
	...
	construct_onion_packet_with_init_noise::<HD, msgs::OnionPacket>(
		payloads, onion_keys, PacketData::Payment(packet_data), Some(associated_data))
}

/// Information used to create an onion packet.
pub(crate) enum PacketData {
	/// We're creating an onion packet for a payment, i.e. to be included in an `update_add_htlc`
	/// message.
	Payment([u8; ONION_DATA_LEN]),
	/// We're creating an onion packet for an onion message.
	Message(Vec<u8>),
}
...
impl PacketData {
	fn len(&self) -> usize {
		match self {
			PacketData::Payment(_) => ONION_DATA_LEN,
			PacketData::Message(data) => data.len(),
		}
	}
}

/// Since onion message packets and onion payment packets have different lengths but are otherwise
/// identical, we use this trait to allow `construct_onion_packet_with_init_noise` to return both
/// types.
pub(crate) trait Packet {
	fn create(pubkey: PublicKey, hop_data: PacketData, hmac: [u8; 32]) -> Self;
}

/// panics if route_size_insane(paylods) and we're sending a payment
fn construct_onion_packet_with_init_noise<HD: Writeable, P: Packet>(mut payloads: Vec<HD>, onion_keys: Vec<OnionKeys>, mut packet_data: PacketData, associated_data: Option<&PaymentHash>) -> P {
	...
	for (i, (payload, keys)) in payloads.iter_mut().zip(onion_keys.iter()).rev().enumerate() {
		...
		payload.write(&mut payload_len).expect("Failed to calculate length");
		match packet_data {
			PacketData::Payment(ref mut arr) => shift_arr_right(arr, payload_len.0 + 32),
			PacketData::Message(ref mut vec) => shift_vec_right(vec, payload_len.0 + 32),
		}

		let mut chacha = ChaCha20::new(&keys.rho, &[0u8; 8]);
		let mut hmac = HmacEngine::<Sha256>::new(&keys.mu);
		macro_rules! process_payload {
			($arr: expr, $payload_len: expr) => {
				$arr[0..payload_len.0].copy_from_slice(&payload.encode()[..]);
				$arr[payload_len.0..($payload_len.0 + 32)].copy_from_slice(&hmac_res);
				chacha.process_in_place($arr);
				if i == 0 {
					$arr[packet_data_len - filler.len()..packet_data_len].copy_from_slice(&filler[..]);
				}
				hmac.input($arr);
			}
		}
		...
		match packet_data {
			PacketData::Payment(ref mut arr) => {
				process_payload!(arr, payload_len);
			},
			PacketData::Message(ref mut vec) => {
				process_payload!(vec, payload_len);
			},
		}
	...
	P::create(onion_keys.first().unwrap().ephemeral_pubkey, packet_data, hmac_res)
}
...
mod tests {
	...
	fn onion_vectors() {
		...
		let packet = super::construct_onion_packet_with_init_noise::<_, msgs::OnionPacket>(payloads, onion_keys, super::PacketData::Payment([0; super::ONION_DATA_LEN]), Some(&PaymentHash([0x42; 32])));
