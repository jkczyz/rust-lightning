        ...
        C::Target: ChainWatchInterface,
{
	...
	watch_events: Mutex<Vec<ChainWatchEvent>>,
	chain_monitor: C,
	broadcaster: T,
	logger: L,
	fee_estimator: F
				...
				for monitor in monitors.values_mut() {
					...
					for (ref txid, ref outputs) in txn_outputs {
						for (idx, output) in outputs.iter().enumerate() {
							...
							watch_events.push(ChainWatchEvent::WatchOutPoint {
								outpoint: OutPoint { txid: txid.clone(), index: idx as u16 },
								script_pubkey: output.script_pubkey.clone()
							});
						}
					}
				}
				reentered = !watch_events.is_empty();
				for event in watch_events.drain(..) {
					log_trace!(self.logger, "event: {:?}", event);
				}
        ...
        C::Target: ChainWatchInterface,
{
	...
	pub fn add_monitor_by_key(&self, key: Key, monitor: ChannelMonitor<ChanSigner>) -> Result<(), MonitorUpdateError> {
		...
		};
		{
			...
			let mut watch_events = self.watch_events.lock().unwrap();
			watch_events.push(ChainWatchEvent::WatchTransaction { txid: funding_txo.0.txid, script_pubkey: funding_txo.1.clone() });
			watch_events.push(ChainWatchEvent::WatchOutPoint { outpoint: funding_txo.0, script_pubkey: funding_txo.1.clone() });
	...
	fn check_spend_remote_transaction<L: Deref>(&mut self, tx: &Transaction, height: u32, logger: &L) -> (Vec<ClaimRequest>, (Txid, Vec<TxOut>)) where L::Target: Logger {
		...
		if commitment_number >= self.get_min_seen_secret() {
			...
			if !claimable_outpoints.is_empty() || per_commitment_option.is_some() { // ie we're confident this is actually ours
				...
				log_trace!(logger, "Duplicate?: {:?}", tx.output.clone());
				...
				if prevout.txid == self.funding_info.0.txid && prevout.vout == self.funding_info.0.index as u32 {
					if (tx.input[0].sequence >> 8*3) as u8 == 0x80 && (tx.lock_time >> 8*3) as u8 == 0x20 {
						...
						if !new_outputs.1.is_empty() {
							log_trace!(logger, "Watching: {:?}", new_outputs);
		...
		if should_broadcast {
			if let Some(commitment_tx) = self.onchain_tx_handler.get_fully_signed_local_tx(&self.funding_redeemscript) {
				...
				if !new_outputs.is_empty() {
					log_trace!(logger, "Watching: {:?}", (self.current_local_commitment_tx.txid.clone(), new_outputs.clone()));
