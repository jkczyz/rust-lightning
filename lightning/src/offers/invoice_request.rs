...
impl<'a> InvoiceRequestBuilder<'a> {
	...
	pub(super) fn deriving_metadata(
		...
		let metadata = Metadata::Derived(MetadataMaterial::new(nonce, expanded_key));
		Self {
			offer,
			invoice_request: Self::create_contents(offer, metadata),
	...
	pub fn build(mut self) -> Result<UnsignedInvoiceRequest<'a>, SemanticError> {
		...
		if unsigned_invoice_request.invoice_request.inner.payer.0.as_bytes().is_none() {
	...
	fn build_without_checks(mut self) -> UnsignedInvoiceRequest<'a> {
		...
		if self.invoice_request.payer.0.material().is_some() {
			let mut metadata = self.invoice_request.payer.0.clone();
		...
		let (metadata, keys) = self.invoice_request.payer.0.into_parts();
		let payer_id = match keys {
			Some(keys) => {
		...
		self.invoice_request.payer.0 = Metadata::Bytes(metadata);
...
mod tests {
	...
	fn builds_invoice_request_with_derived_metadata() {
		...
		let offer = OfferBuilder::new("foo".into(), recipient_pubkey())
			.amount_msats(1000)
			.build().unwrap();
		let invoice_request = offer.request_invoice_deriving_metadata(node_id, &expanded_key, nonce)
			.unwrap()
			.build().unwrap()
			.sign(payer_sign).unwrap();
		let encrypted_nonce = EncryptedNonce::try_from(
			&invoice_request.metadata()[..EncryptedNonce::LENGTH]
		).unwrap();
	...
	fn builds_invoice_request_with_derived_payer_id() {
		...
		let offer = OfferBuilder::new("foo".into(), recipient_pubkey())
			.amount_msats(1000)
			.build().unwrap();
		let unsigned_invoice_request = offer.request_invoice_deriving_payer_id(&expanded_key, nonce)
			.unwrap()
			.build().unwrap();

		let keys = unsigned_invoice_request.keys.unwrap();
		let secp_ctx = Secp256k1::new();
		let invoice_request = unsigned_invoice_request
			.sign::<_, Infallible>(|digest| Ok(secp_ctx.sign_schnorr_no_aux_rand(digest, &keys)))
			.unwrap();
		let encrypted_nonce = EncryptedNonce::try_from(invoice_request.metadata()).unwrap();
		assert_eq!(encrypted_nonce.decrypt_for_offer(&expanded_key), nonce);
